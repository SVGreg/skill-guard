...
| `SG-EVA-003` | **Document-mediated multimodal payload** — the instructions live *inside a bundled image* (logo / workflow chart / screenshot) and the prose is rewritten to reference that image as a normal workflow step; the agent reads it with its vision tool at execution time | completes the `SG-EVA-*` set: 001 hides the payload by **location**, 002 by **encoding**, 003 by **modality**. skill-guard cannot read the image (`classify` files it as an inert `asset`) and should not try — the rule is the **pointer**, which the attack cannot omit, the same asymmetry `SG-EVA-001` exploits with its decoder. Source: SkillCamo, arXiv 2606.18198 |
